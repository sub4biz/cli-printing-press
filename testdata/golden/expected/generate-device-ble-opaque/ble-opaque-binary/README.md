...
- `ble-opaque-binary-pp-cli doctor` reports whether the live backend is compiled in and, with `--live`, whether the device is reachable. `ble-opaque-binary-pp-cli scan --live` lists nearby devices by service UUID.
